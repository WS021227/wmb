...
$(".class-content .tab .tab-box").click(function () {
    console.log($(this).index(), "787878787")
    if ($(this).index() == 1) {
        nb_page_flag = 'sjsm'
...
function get_tab_content_data(id) {
    ...
    axios.get(`/async/new/tutorial/list?start=0&category=4&source=${nb_page_flag}&category_child=${id}`).then(function (res) {
...
$(".tg-content .tg-content-more").click(function () {
    nb_start_num += 15
    if (nb_tab_index == 0) {
        axios.get(`/async/new/tutorial/list?start=${nb_start_num}&category=4&source=${nb_page_flag}`).then(function (res) {
            console.log(res.data.content)
            if ($(res.data.content).data("id") == 'no-data') {
        ...
        axios.get(`/async/new/tutorial/list?start=${nb_start_num}&category=4&source=${nb_page_flag}&category_child=${nb_tab_id}`).then(function (res) {
            console.log(res.data.content)
            if ($(res.data.content).data("id") == 'no-data') {
                $(".tg-content>.tg-content-more").css("display", "none")
            }
