function page_ctrl(data_obj) {
  ...
  var total_page=Math.ceil(total_item/per_num);//计算总页数,
  var inquire_list=data_obj.seach_list
  if(total_page<1){
    return;
  }

  $(obj_box).append('<div class="page_ctrl"></div>');
  function page_even(el) {
    /*加载数据*/
   

if(el>1){
  inquire_list.start=el
}
console.log(inquire_list.start,'search_list43223')

$.ajax('/buyer/search_list', {
  datatype: 'text/json',
  type: 'get',
  data: inquire_list,
  success: function(result) {
    console.log(result,)
    $('.sousou-list').html(result)
    
    // $.each(result.data.list, function() {
    //   $('.history-keys').append('<span onclick="searchKey(this)"  data-keywords=' + this
    //     .keywords + '>' + this.keywords + '</span>')
    // })
  }
})
    ...
    for(var i=0;i<total_page-1;i++){
      if(total_page>4&&current_page>2&&i<current_page-1){
        if(i<1){
	   ...
	   if(total_page>4&&current_page<total_page-3&&i>current_page+1){
        if(current_page>4&&i==current_page+2){
          append_html+='<span class="page_dot">•••</span>';
    ...
    append_html+='<button class="next_page">下一页</button><span class="page_total">'+current_page+'/ '+total_page+' , 到第</span><input class="input_page_num" type="text" value="'+inp_val+'"><span class="page_text">页</span><button class="to_page_num">确定</button>';
