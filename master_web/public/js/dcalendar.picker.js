...
+function ($) {
	...
	var months_lang = {
			...
			'us-en': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
		}, months = [],
		short_months_lang = {
			'zh-cn': ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'],
			'us-en': ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
		}, short_months =[],
		daysofweek_lang = {
			'zh-cn': ['星期天', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'],
			'us-en': ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
		}, daysofweek = [],
        btn_clear = {'zh-cn': '清空', 'us-en':'clear'}
		DCalendar = function (elem, options) {
			...
			this.viewMode = options.lastViewMode || 'days';
			...
			this.calendar.on('click', '#next', function () {
				...
				.on('click', '#today', function () {
					console.log(that.options.lastViewMode, 'that.options.lastViewMode')
					if(that.options.lastViewMode == 'months') {
						var curr = new Date(that.date),
							cmonth = that.date.getMonth(),
							cyear = curr.getFullYear()
						that.selected = cmonth + '/01/' + cyear;
						that.selectDate();
						return
					}
					that.viewMode = 'days';
					...
					that.selected = (that.date.getMonth()+1) + '/'+ that.date.getDate() +'/' + that.date.getFullYear();
					that.create('days');
					that.selectDate();
				})
				...
				.on('click', '.month', function (e) {
					if(that.options.lastViewMode == 'months'){
						var curr = new Date(that.date),
							cmonth = Number($(e.currentTarget).attr('num')) + 1,
							cyear = curr.getFullYear()
						that.selected = cmonth + '/01/' + cyear;
						that.selectDate();
					}else {
				...
				.on('click', '#clear', function (e) {
					that.calendar.trigger($.Event('cleardate'));
				});
	...
	DCalendar.prototype = {
		...
		create: function (mode) {
			...
			if (mode === "days") {
				...
				console.log(that.options.mode, 'that.options.mode')
				...
				for (var i = 1; i <= 6; i++) {
					...
					while (day <= days) {
						...
						if(that.options.mode === 'datepicker' && d.getTime() == selected.getTime()){
							console.log(that.options.mode === 'datepicker' && d.getTime() == selected.getTime(), d, selected ,'qweqw')
						}
						if (that.options.mode === 'datepicker' && d.getTime() == selected.getTime()) temp[dayOfWeek].addClass('selected');

						if (i === 1 && dayOfWeek === 0) break;
						else if (dayOfWeek < 6) temp[dayOfWeek].html('<span>' + (day++) + '</span>').addClass('date  dd1');
						else {
							temp[dayOfWeek].html('<span>' + (day++) + '</span>').addClass('date dd2');
							break;
						}
					}
						...
						if (i === 1) {
							...
							for (var a = 6; a >= 0; a--) {
								if (temp[a].text() === '') {
									...
									if(that.options.mode === 'datepicker' && d.getTime() == selected.getTime()){
										console.log(that.options.mode === 'datepicker' && d.getTime() == selected.getTime(), d, selected ,'qweqw111')
									}
								}
							}
						}
		 ...
		 * @private
		 */
		_curr_dt: function(tBody, colspan){
			// 无今日按钮
			if(this.options.no_today) return;
			if(this.options.lastViewMode == 'months') {
				var sysDate = "This Month: " + months[this.today.getMonth()] + "  " + this.today.getFullYear();
				tBody.append('<tr><td colspan="' + colspan + '" id="today">' + sysDate + '</td></tr>').appendTo(this.calendar);
			}else{
				var sysDate = "Today: " + daysofweek[this.today.getDay()] + ", " + months[this.today.getMonth()] + " " + this.today.getDate() + ", " + this.today.getFullYear();
				tBody.append('<tr><td colspan="' + colspan + '" id="today">' + sysDate + '</td></tr>').appendTo(this.calendar);
			}
		},
		_clear: function(tBody, colspan){
			tBody.append('<tr><td colspan="'+ colspan +'" id="clear">'+ btn_clear[this.options.language] +'</td></tr>').appendTo(this.calendar);
		},
		_months: function () {
			months = months_lang[this.options.language]
		},
		_short_months: function () {
			short_months = short_months_lang[this.options.language]
		},
		_weeks: function () {
			daysofweek = daysofweek_lang[this.options.language]
		}
	}
	...
	$.fn.dcalendarpicker = function (opts) {
        return $(this).each(function () {
            ...
            if (typeof opts === 'string') {
				...
				let mode = opts.mode || 'datepicker'
                if (opts) {
                    opts.mode = mode;
                    cal.dcalendar(opts);
                } else {
                    cal.dcalendar({mode: mode});
                }
                cal.hover(function () {
                    hovered = true;
                }, function () {
                    ...
                    that.val('').trigger('onchange');
                    that.trigger($.Event('dateselected', {date: '', elem: that}));
                });
                that.on('keydown', function (e) {
                    if (e.which && opts.disable_keydown) return false;
                })
                    .on('focus', function () {
