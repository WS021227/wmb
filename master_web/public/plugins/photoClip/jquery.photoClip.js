...
(function(root, factory) {
	...
	if (typeof define === "function" && define.amd) {
		define(["jquery", "public/plugins/photoClip/iscroll-zoom", "public/plugins/photoClip/hammer", "lrz"], factory);
	} else if (typeof exports === "object") {
		module.exports = factory(require("jquery"), require("public/plugins/photoClip/iscroll-zoom"), require("public/plugins/photoClip/hammer"), require("lrz"));
	} else {
	...
	function photoClip(container, option) {
		...
		$file.change(function() {
			...
			if (!/image\/\w+/.test(files.type)) {
				...
				fileReader.onload = function(e) {
					lrz(files)
					.then(function (rst) {
						var org_width = rst.origin.exifdata.PixelXDimension,
							ori_height = rst.origin.exifdata.PixelYDimension
						if(org_width >= 3264 || ori_height >= 2448){
							return loadError.call(this, 'Image size is too large (width cannot exceed 3264, height cannot exceed 2448)');
						}
						// 处理成功会执行
		...
		function clipImg() {
			...
			let tx = curX - local.x / scale, ty = curY - local.y / scale
			ctx.translate(tx, ty);
			ctx.rotate(curAngle * Math.PI / 180);

			ctx.drawImage($img[0], 0, 0);
			ctx.restore();
			let result_local = {width: canvas.width, height: canvas.height, x: Math.abs(tx), y: Math.abs(ty)}
			var dataURL = canvas.toDataURL(outputType, 1);
			$view.css("background-image", "url("+ dataURL +")");
			clipFinish.call($img[0], result_local, dataURL);
		}
